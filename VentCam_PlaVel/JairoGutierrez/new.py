...
def main():
    ...
    for i in range(10):
        ...
        while intentos < intentos_max:
            ...
            # --- Nueva lógica de ordenamiento de correos ---
            log("Verificando y estableciendo el orden de los correos...")

            # Intentar encontrar si ya está ordenado por "De más antiguo a más reciente"
            if hacer_clic_en_imagen("imagenes/filtrado_de_antiguo_a_reciente.png", "Verificar si ya está ordenado de antiguo a reciente", tiempo_espera=1):
                log("Los correos ya están ordenados de más antiguo a más reciente. No se necesita acción.")
            # Si está ordenado de "reciente a antiguo", hacer clic para invertir el orden.
            elif hacer_clic_en_imagen("imagenes/filtrado_de_reciente_a_antiguo.png", "Verificar si está ordenado de reciente a antiguo", tiempo_espera=1):
                log("Cambiando orden de 'reciente a antiguo' a 'antiguo a reciente'.")
                # No es necesario hacer más clics, ya que al hacer clic en el estado se invierte.
            else:
                # Si no se encuentra ninguno de los estados de filtro por fecha, se asume que no está ordenado por fecha.
                log("No se detectó orden por fecha. Se procederá a configurar el ordenamiento.")
                
                # Buscar el menú desplegable de ordenamiento (puede ser cualquiera de los dos iconos de filtro)
                pos_filtro = hacer_clic_en_imagen("imagenes/filtrado_de_reciente_a_antiguo.png", "Buscando menú de ordenamiento (opción 1)", tiempo_espera=1)
                if not pos_filtro:
                    pos_filtro = hacer_clic_en_imagen("imagenes/filtrado_de_antiguo_a_reciente.png", "Buscando menú de ordenamiento (opción 2)", tiempo_espera=1)
                
                # Si se encontró el menú, se hace clic a la derecha para abrir las opciones.
                if pos_filtro:
                    log("Menú de ordenamiento encontrado. Abriendo opciones...")
                    pyautogui.moveRel(40, 0, duration=0.2) # Mover 40px a la derecha
                    pyautogui.click()
                    time.sleep(1) # Esperar a que aparezca el menú
                else:
                    # Como último recurso, si no se encuentra el menú de filtro, se intenta hacer clic en "Organizar por"
                    log("No se encontró el menú de filtro. Intentando hacer clic en 'Organizar por'.")
                    if not hacer_clic_en_imagen("imagenes/organizar_por.png", "Botón 'Organizar por'", tiempo_espera=2):
                        limpiar_estado_o_cerrar("No se pudo encontrar el control para ordenar los correos.")

                # Una vez que el menú está (o debería estar) abierto, se selecciona "Fecha"
                if not hacer_clic_en_imagen("imagenes/cambiar_orden_de_correos_por_fecha.png", "Seleccionar 'Fecha' en el menú de ordenamiento", tiempo_espera=2):
                    limpiar_estado_o_cerrar("No se pudo encontrar la opción 'Fecha' para ordenar.")
                
                # Finalmente, se asegura de que el orden sea "De más antiguo a más nuevo"
                if not hacer_clic_en_imagen("imagenes/organizar_correos_del_mas_antiguo_al_mas_nuevo.png", "Seleccionar 'De más antiguo a más nuevo'", tiempo_espera=2):
                    log("No se pudo seleccionar 'De más antiguo a más nuevo', es posible que ya estuviera seleccionado o que el orden se invirtiera con el clic anterior.")
